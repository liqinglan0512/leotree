...
const missingGeneratedDocs = !existsSync(join(TEMPLATE_ROOT, "AGENTS.md")) || !existsSync(join(TEMPLATE_ROOT, ".grok/skills/og/SKILL.md"));
const generatedDocOptions = { skip: missingGeneratedDocs ? "Review bundle intentionally omits platform-generated AGENTS/OG skill documents" : false };
const readDoc = (rel) => readFileSync(join(TEMPLATE_ROOT, rel), "utf8");

test("SKILL.md and AGENTS.md name the marker path and bound this script uses", generatedDocOptions, () => {
...
test("the sections that own the brand-task prohibition never affirm a wait", generatedDocOptions, () => {
...
test("SKILL.md tells the pass to self-check with the flag this CLI accepts", generatedDocOptions, () => {
